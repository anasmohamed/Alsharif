...
class AddVillaVC: UIViewController,AlertsPresenting,CLLocationManagerDelegate {
    ...
    @IBOutlet weak var villa: DropDown!
    @IBOutlet weak var numberApartmentsTF: MainTF!
    ...
    @IBOutlet weak var numberKitchensTF: MainTF!

    @IBOutlet weak var villaAgeTF: MainTF!
    ...
    @IBOutlet weak var villaImage1: UIImageView!
    @IBOutlet weak var villaImage2: UIImageView!
    @IBOutlet weak var villaImage360: UIImageView!
    ...
    func dropDown() {
        ...
        villa.optionArray = ["دبلوكس","دور واحد","دورين"]
    ...
    @IBAction func saveAddVillaPressed(_ sender: UIButton) {
        do{
        ...
        let contract = try contractTF.validatedText(validationType: .requiredField(field: "الرجاء اختيار نوع العقد "))
            ...
            let storage = Storage.storage().reference(forURL: "gs://fatenaapp-6a454.appspot.com")
            ...
            self.saveVilla(VillaModel.init(EstateType: "villa", title: title, contract: contract, payment: payment, apartment:apartmentType , furnished: furnished, carPark: carPark, elevator: elevator, floor: floor, room: room, hall: hall, toilet: toilet, apartmentArea: apartmentArea, tank: tank, electricity: electricity, frontispiece: frontispiece, streetWeight: streetWeight, contact: contact, price: price, details: details, longitude: self.selectedLocation.longitude, latitude: self.selectedLocation.latitude , image360: "", image1: "", image2: metaImageUrl))
    ...
    func saveVilla(_ villa:VillaModel){
        let docData:[String:Any] = [
            "EstateType":villa.EstateType,
            "title":villa.title,
            "contract":villa.contract,
            "payment":villa.payment,
            "apartment":villa.apartment,
            "furnished":villa.furnished,
            "carPark":villa.carPark,
            "elevator":villa.elevator,
            "floor":villa.floor,
            "room":villa.room,
            "hall":villa.hall,
            "toilet":villa.toilet,
            "apartmentArea":villa.apartmentArea,
            "tank":villa.tank,
            "electricity":villa.electricity,
            "frontispiece":villa.frontispiece,
            "streetWeight":villa.streetWeight,
            "contact":villa.contact,
            "price":villa.price,
            "details":villa.details,
            "longitude":villa.longitude,
            "latitude":villa.latitude,
            "image360":villa.image360,
            "image1":villa.image1,
            "image2":villa.image2,
        ]
...
extension AddVillaVC: ImagePickerDelegate {
    func didSelect(image: UIImage?) {
        if let image = image {
            DispatchQueue.main.async {
                self.villaImage1.image  = image
                self.villaImage1.contentMode = .scaleAspectFill
            }
        }
    }
}

extension AddVillaVC:Storyboarded{
    static var storyboardName: StoryboardName = .main
    
}
extension AddVillaVC: GMSMapViewDelegate {
